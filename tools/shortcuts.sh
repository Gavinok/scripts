...
vim_shortcuts="${XDG_CONFIG_HOME}/vimshortcuts.vim"
...
sed "s/\s*#.*$//;/^\s*$/d" "$HOME/.config/bmfiles" | tee >(awk '{print "alias "$1"=\"$EDITOR "$2"\" "}' >>"$shell_shortcuts") \
	...
	>(awk '{print "nmap <leader>"$1, ":e", $2 "<CR>"}' >>"$vim_shortcuts") |
